...
/* ============================================================
   ...
   ============================================================ */
/* ============================================================
   TAB 9: LEGACY — Jack vs Jim Kingston (regenerated by derive_legacy_data.py)
   ============================================================ */
// Jack: FEC Schedule A, committee C00261958, 2004-2012 House cycles, in 2026 dollars
...
const TabLegacy = () => {
  ...
  const perCycle = jk.total / 5;                  // Jack averaged over 5 House cycles
  ...
  const tierData = [
    { tier: 'High', Jack: jk.tiers.High, Jim: jm.tiers.High },
    { tier: 'Upper-mid', Jack: jk.tiers.UpperMid, Jim: jm.tiers.UpperMid },
    { tier: 'Middle', Jack: jk.tiers.Middle, Jim: jm.tiers.Middle },
  ...
  const Bars = ({ data, xKey, fmt }) => (
    <ResponsiveContainer width="100%" height={250}>
      <BarChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 4 }} barGap={3}>
        <CartesianGrid strokeDasharray="2 4" stroke={P.line} vertical={false}/>
        <XAxis dataKey={xKey} tick={{ fontFamily: 'DM Sans', fontSize: 11, fill: P.ink, fontWeight: 600 }} axisLine={{ stroke: P.line }} tickLine={false}/>
        <YAxis tickFormatter={(v) => v + '%'} tick={{ fontFamily: 'DM Sans', fontSize: 10, fill: P.muted }} axisLine={false} tickLine={false}/>
        <Tooltip content={<TTip/>}/>
        <Legend wrapperStyle={{ fontFamily: 'DM Sans', fontSize: 12, paddingTop: 6 }}/>
        <Bar name="Jack (2004–12)" dataKey="Jack" fill={JACKC} radius={[4,4,0,0]}/>
        <Bar name="Jim (2026)" dataKey="Jim" fill={P.kingston} radius={[4,4,0,0]}/>
      </BarChart>
    </ResponsiveContainer>
  );
  ...
  return (
    <div>
      ...
      <Card style={{ padding: 26, marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.4fr 1fr', gap: 18 }}>
          ...
          <div style={{ display: 'grid', gridTemplateRows: 'repeat(2, 1fr)', gap: 12 }}>
            <div style={{ padding: 16, background: P.bg, borderRadius: 10 }}>
              <div style={{ fontSize: 11, color: P.muted, textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 700 }}>Weighted-avg donor ZIP income</div>
              <div style={{ marginTop: 6, fontFamily: 'Fraunces, serif', fontSize: 22, fontWeight: 600 }}><span style={{ color: P.kingston }}>${jm.wAvg.toLocaleString()}</span> <span style={{ color: P.muted, fontSize: 14 }}>Jim</span></div>
              <div style={{ fontFamily: 'Fraunces, serif', fontSize: 22, fontWeight: 600 }}><span style={{ color: JACKC }}>${jk.wAvg.toLocaleString()}</span> <span style={{ color: P.muted, fontSize: 14 }}>Jack</span></div>
            </div>
            <div style={{ padding: 16, background: P.bg, borderRadius: 10 }}>
              <div style={{ fontSize: 11, color: P.muted, textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 700 }}>Share from wealthy GA ZIPs (HHI ≥ $90K)</div>
              <div style={{ marginTop: 6, fontFamily: 'Fraunces, serif', fontSize: 22, fontWeight: 600 }}><span style={{ color: P.kingston }}>{jm.wealthyPct}%</span> <span style={{ color: P.muted, fontSize: 14 }}>Jim</span></div>
              <div style={{ fontFamily: 'Fraunces, serif', fontSize: 22, fontWeight: 600 }}><span style={{ color: JACKC }}>{jk.wealthyPct}%</span> <span style={{ color: P.muted, fontSize: 14 }}>Jack</span></div>
            </div>
          </div>
        </div>
      </Card>
      ...
      {/* CAVEATS */}
      <Card tone="warning" style={{ padding: '18px 22px', marginBottom: 16 }}>
